...
export async function GET(request: Request) {
  ...
  try {
    ...
    const response_redirect = NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL}?source=notion&workspaceName=${encodeURIComponent(workspace_name)}&workspaceId=${workspace_id}`
    );
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_BASE_URL}?error=notion_auth_failed`);
  }
}
